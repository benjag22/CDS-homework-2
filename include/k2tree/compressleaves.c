...
int compress_leaves(int argc, const char *argv[]) {
    FILE *f;
    uint nodes;
    ulong edges;
    register ulong i;
    ...
    for (fila = 0; fila < _part; fila++) {
        for (columna = 0; columna < _part; columna++) {
            ...
            _partleavesInf[fila][columna] = (uint *) malloc(
                sizeof(uint) * _partnleaves[fila][columna] * (K2_2 * K2_2 + WORD - 1) / WORD
            );
            fread(
                _partleavesInf[fila][columna],
                sizeof(uint),
                (_partnleaves[fila][columna] * K2_2 * K2_2 + WORD - 1) / WORD,
                fvil
            );
        }
    }
