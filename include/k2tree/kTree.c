...
//				fprintf(stderr,"row= %d, column=%d, edges from %d and from %d, (%2.2f\%)\n",fila,columna,fila*subm, columna*subm,(float)(fila*part+columna)*100/part/part);
		...
		for(i=0;i<trep->maxLevel1+trep->maxLevel2;i++){
			...
			if(i==trep->maxLevel1+trep->maxLevel2-1){
				...
				bits_BN = (uint*)malloc(sizeof(uint)*((bits_BN_len/WORD+1)));
				for(j=0; j<(bits_BN_len/WORD+1);j++)
					bits_BN[j]=0;
				bits = bits_BN;
			...
			uint * bits_LI = (uint*)malloc(sizeof(uint)*((bits_LI_len+WORD-1)/WORD));
			for(i=0; i<(WORD-1+bits_LI_len)/WORD;i++)
				bits_LI[i]=0;
		...
		//PARTIAL SAVE
			...
			if(rep->numberOfEdges>0){
				...
				if(rep->numberOfEdges==0)
					return;
				fwrite(rep->leavesInf,sizeof(uint),(rep->nleaves*K2_2*K2_2+WORD-1)/WORD,_fil);
				free(rep->leavesInf);
			}
...
MREP * loadRepresentation(char * basename){
	...
	rep->leavesInf = (uint *)malloc(sizeof(uint)*(rep->nleaves*K2*K2/WORD+1));
	fread(rep->leavesInf,sizeof(uint),rep->nleaves*K2*K2/WORD+1,fi);
	fclose(fi);   
...
MREP * createRepresentation(NODE * root, uint numberOfNodes,ulong numberOfEdges){
	...
	bitRankW32Int *BT, *BN;
	uint * bits_BT = (uint*)malloc(sizeof(uint)*((bits_BT_len+WORD-1)/WORD));
	uint * bits_BN = (uint*)malloc(sizeof(uint)*((bits_BN_len+WORD-1)/WORD));
	uint * bits_LI = (uint*)malloc(sizeof(uint)*((bits_LI_len+WORD-1)/WORD));
	

	
	int i, k, j, queuecont, conttmp,node,div_level, pos=0;
	for(i=0; i<(WORD-1+bits_BT_len)/WORD;i++)
		bits_BT[i]=0;
	for(i=0; i<(WORD-1+bits_BN_len)/WORD;i++)
		bits_BN[i]=0;
	for(i=0; i<(WORD-1+bits_LI_len)/WORD;i++)
		bits_LI[i]=0;
	...
	//GRABAR IL sin comprimir
				...
				for(fila=0;fila<part;fila++){
					for(columna=0;columna<part;columna++){  
						...
						if(trep->submatrices[fila][columna]->numberOfEdges==0)
							continue;
						fwrite(rep->leavesInf,sizeof(uint),(rep->nleaves*K2_2*K2_2+WORD-1)/WORD,fvil);
